...
namespace OrchardCore.Commerce.Drivers
{
    public class ProductPartDisplayDriver : ContentPartDisplayDriver<ProductPart>
    {
        ...
        public override async Task<IDisplayResult> UpdateAsync(ProductPart model, UpdatePartEditorContext context)
        {
            await context.Updater.TryUpdateModelAsync(model, Prefix, t => t.Sku);
            ...
            if (hasSku)
            {
                context.Updater.ModelState.BindValidationResult(Prefix, new ValidationResult("Existing SKU choose another", new[] { nameof(model.Sku) }));
            }
        ...
        private async Task BuildViewModel(ProductPartViewModel model, ProductPart part)
        {
            ...
            model.Attributes = await _productAttributeService.GetProductAttributeFieldsAsync(part.ContentItem);
